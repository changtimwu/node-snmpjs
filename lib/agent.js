...
Agent.prototype._do_getset = function _do_getset(req, rsp) {
	...
	req.pdu.varbinds.forEach(function (vb, i) {
      ...
      console.log("prq.instance=", prq.instance);
      prq.instance = prq.instance ? prq.instance[0]:0
      if (node.columns) {
        if ( !(prq.instance in node.columns))
          reject=true
      }else if (prq.instance>0){
        reject=true
      }
      if (reject) {
				handler = [ function _getset_nsihandler(pr) {
					var rsd = data.createData({ type:'Null', value:data.noSuchInstance});
					var rsvb = varbind.createVarbind({ oid: pr.oid, data: rsd });
					pr.done(rsvb);
				} ];
			} else {
